...
function renderDoctorCard(s){const total=s.slots.reduce((a,b)=>a+b.availableQuota,0);const times=s.slots.map(slot=>`${slot.startTime}-${slot.endTime}（剩余号源：${slot.availableQuota}）`).join('<br>');document.getElementById('doctorGrid').insertAdjacentHTML('beforeend',`<div class="card doctor-card" onclick="openSchedule(${s.id})"><h3>${s.doctorName}</h3><p class="muted">${s.departmentName} / ${s.period} / ${s.scheduleDate}</p><p>可选时间：</p><p>${times||'暂无具体时段'}</p><p><b>总剩余号源：${total}</b></p><p class="muted">点击卡片查看并预约具体时间</p></div>`);}
