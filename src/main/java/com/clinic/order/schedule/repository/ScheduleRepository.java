package com.clinic.order.schedule.repository;
...
public class ScheduleRepository {
    ...
    public List<ScheduleSlot> findSlotsByScheduleId(Long scheduleId) {
        return jdbcTemplate.query("SELECT * FROM schedule_slot WHERE schedule_id = ? ORDER BY start_time",
                (rs, rowNum) -> mapSlot(rs), scheduleId);
    }

    public ScheduleSlot findSlot(Long slotId) {
        return jdbcTemplate.queryForObject("SELECT * FROM schedule_slot WHERE id = ?", (rs, rowNum) -> mapSlot(rs), slotId);
    }

    public int decreaseMysqlQuota(Long slotId) {
        return jdbcTemplate.update("""
            UPDATE schedule_slot SET available_quota = available_quota - 1, version = version + 1
